...
var data_arr = [];
var data_number = 0;
var mediaOpts = { video: { width: 400, height: 180 } };
var medstream = null;
var canvas;
var ctx;
var take_camera;
...
function dr_rc(x1, y1, x2, y2) {
    ...
    if(is_model == 0 || is_model == 1) {
        ...
        let roi_txt = 'ROI:[' + x1 * Hc + ', ' + y1 * Vc + ', ' + x2 * Hc + ', ' + y2 * Vc + ']';
   ...
   */
    box.style.opacity = '1';
    box.style.left = parseInt(ico.style.left) + left + 'px';
    box.style.top = parseInt(ico.style.top) + top + 'px';
...
function send_camera() {
            ctx.drawImage(video, 0, 0, 400, 180);
...
function start() {
    ...
    if(up_model == false) {
        log('Please upload the module first', true);
        return;
    }
    ...
    if(is_model == 0) {
        ...
        ico.style.left = '160px';
        ico.style.top = '10px';
        ico.style.width = '400px';
        ico.style.height = '180px';
        ico.style.opacity = '1';
...
function upload_model() {
    ...
    var lineReader = require('readline').createInterface({
        input: require('fs').createReadStream(model_path.value)
      });

      lineReader.on('line', function (line) {
        let buffer = new ArrayBuffer(256);
        let view = new Uint8Array(buffer, 0, 255);
        ...
        if (index == 1) {
            ...
            for (let i = 0; i < line.length; ++i) {
                view[i] = line[i].charCodeAt();
            }
            send_data(7, 0, line.length, 0, view, line.length);
        }
        if(index == 2 || index == 3) {
            output_par += line;
            if(index == 3) {
                for (let i = 0; i < output_par.length; ++i) {
                    view[i] = output_par[i].charCodeAt();
                }
                send_data(8, 0, output_par.length, 0, view, output_par.length);
            }
            output_par += '\n';
        }
        if(index == 4) {
            ...
            send_data(9, 0, line.length, 0, view, line.length);
            {
                let model = model_path.value.substring(0,model_path.value.lastIndexOf('\\')+1);
                model += model_name;
                send_file(1, model, 0);
                up_model = true;
