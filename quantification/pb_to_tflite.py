...
def representative_dataset_gen():
    for img in img_lists:
        jpg = img.endswith(".jpg")
        if jpg == True:
            img = cv2.imread(sys.argv[1] + "/" +img)
...
# 定义转换器
converter = tf.lite.TFLiteConverter.from_frozen_graph(sys.argv[2], ["Input"], ["Identity"], {"Input":[1,56,56,3]})
# 定义量化配置
...
open(sys.argv[3], "wb").write(quantize_model)
